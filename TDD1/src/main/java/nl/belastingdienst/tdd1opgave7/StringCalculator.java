...
public class StringCalculator {
    ...
    public int add(String input){
        ...
        int sum = Arrays.stream(input.split(delimiter)).map(String::strip).filter((String s) -> { if(!s.contains("-")) return !s.isEmpty() && !s.contains("/"); else negatives.add(s); return false; }).mapToInt(Integer::parseInt).filter(n -> n <= MAX_INT).sum();
